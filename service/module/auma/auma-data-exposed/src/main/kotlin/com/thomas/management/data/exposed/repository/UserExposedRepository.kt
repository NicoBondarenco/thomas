...
class UserExposedRepository(
    ...
    override fun one(
        id: UUID
    ): UserGroupsEntity? = transacted { findUser(id)?.toUserCompleteEntity() }
    ...
    ): UserGroupsEntity = entity.upsert {
    ...
    private fun UserGroupsEntity.upsert(
        exposedEntity: (UserGroupsEntity) -> Unit
    ) = transacted {
