...
import com.thomas.management.data.entity.UserBaseEntity.Companion.MAX_NAME_SIZE
import com.thomas.management.data.entity.UserBaseEntity.Companion.MIN_NAME_SIZE
...
abstract class UserEntityBaseTest<T : UserBaseEntity> {
    ...
    abstract fun entity(
        ...
        mainEmail: String = "tony.stark@email.com",
        documentNumber: String = "58370027075",
        phoneNumber: String? = "5516988776655",
        profilePhoto: String? = null,
        birthDate: LocalDate? = LocalDate.of(1990, 4, 28),
        userGender: Gender? = CIS_MALE,
    ...
    fun `WHEN first name has less than 2 characters THEN should throws EntityValidationException`() {
        val exception = assertThrows<EntityValidationException> { entity(firstName = "A") }
        ...
        val field = UserBaseEntity::firstName.name.toSnakeCase()
    ...
    fun `WHEN first name has more than 250 characters THEN should throws EntityValidationException`() {
        val exception = assertThrows<EntityValidationException> { entity(firstName = "A".repeat(251)) }
        assertEquals(managementUserValidationInvalidEntityErrorMessage(), exception.message)
        val details = (exception.detail as? Map<String, List<String>>)!!
        val field = UserBaseEntity::firstName.name.toSnakeCase()
        assertTrue(details.containsKey(field))
    ...
    fun `WHEN first name has invalid character THEN should throws EntityValidationException`(char: Char) {
        val exception = assertThrows<EntityValidationException> { entity(firstName = "Tony$char") }
        assertEquals(managementUserValidationInvalidEntityErrorMessage(), exception.message)
        val details = (exception.detail as? Map<String, List<String>>)!!
        val field = UserBaseEntity::firstName.name.toSnakeCase()
        assertTrue(details.containsKey(field))
    ...
    fun `WHEN last name has less than 2 characters THEN should throws EntityValidationException`() {
        val exception = assertThrows<EntityValidationException> { entity(lastName = "A") }
        assertEquals(managementUserValidationInvalidEntityErrorMessage(), exception.message)
        val details = (exception.detail as? Map<String, List<String>>)!!
        val field = UserBaseEntity::lastName.name.toSnakeCase()
        assertTrue(details.containsKey(field))
    ...
    fun `WHEN last name has more than 250 characters THEN should throws EntityValidationException`() {
        val exception = assertThrows<EntityValidationException> { entity(lastName = "A".repeat(251)) }
        assertEquals(managementUserValidationInvalidEntityErrorMessage(), exception.message)
        val details = (exception.detail as? Map<String, List<String>>)!!
        val field = UserBaseEntity::lastName.name.toSnakeCase()
        assertTrue(details.containsKey(field))
    ...
    fun `WHEN last name has invalid character THEN should throws EntityValidationException`(char: Char) {
        val exception = assertThrows<EntityValidationException> { entity(lastName = "Stark$char") }
        assertEquals(managementUserValidationInvalidEntityErrorMessage(), exception.message)
        val details = (exception.detail as? Map<String, List<String>>)!!
        val field = UserBaseEntity::lastName.name.toSnakeCase()
        assertTrue(details.containsKey(field))
    ...
    fun `WHEN email has invalid format THEN should throws EntityValidationException`(email: String) {
        val exception = assertThrows<EntityValidationException> { entity(mainEmail = email) }
        assertEquals(managementUserValidationInvalidEntityErrorMessage(), exception.message)
        val details = (exception.detail as? Map<String, List<String>>)!!
        val field = UserBaseEntity::mainEmail.name.toSnakeCase()
        assertTrue(details.containsKey(field))
    ...
    fun `WHEN document number is invalid THEN should throws EntityValidationException`(document: String) {
        val exception = assertThrows<EntityValidationException> { entity(documentNumber = document) }
        assertEquals(managementUserValidationInvalidEntityErrorMessage(), exception.message)
        val details = (exception.detail as? Map<String, List<String>>)!!
        val field = UserBaseEntity::documentNumber.name.toSnakeCase()
        assertTrue(details.containsKey(field))
    ...
    fun `WHEN phone number is invalid THEN should throws EntityValidationException`(phone: String) {
        val exception = assertThrows<EntityValidationException> { entity(phoneNumber = phone) }
        assertEquals(managementUserValidationInvalidEntityErrorMessage(), exception.message)
        val details = (exception.detail as? Map<String, List<String>>)!!
        val field = UserBaseEntity::phoneNumber.name.toSnakeCase()
        assertTrue(details.containsKey(field))
